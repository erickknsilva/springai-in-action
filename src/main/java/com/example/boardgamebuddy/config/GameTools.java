...
import com.example.boardgamebuddy.domain.dto.GameComplexityRequest;
...
public class GameTools  implements Function<GameComplexityRequest, GameComplexityResponse> {
    ...
    private final static Logger LOGGER = LoggerFactory.getLogger(GameTools.class);
    ...
    public GameComplexityResponse apply(GameComplexityRequest gameDataRequest) {
        ...
        String gameSlug = gameDataRequest.title().toLowerCase ().replace(" ", "_");

        LOGGER.info("Getting complexity for {} ({})", gameDataRequest.title(), gameSlug);
        Optional<Game> gameOpt = gameRepository.findBySlug(gameSlug);

        var game = gameOpt.orElseGet(()-> {
            LOGGER.warn("Game not found: {}", gameSlug);
            return new Game(null, gameSlug, gameDataRequest.title(), GameComplexity.UNKNOWN.getValue());
        });
